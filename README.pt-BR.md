# Arquitetura como Código

[English](./README.md) | [简体中文](./README.zh-CN.md) | [日本語](./README.ja.md)

Visualize, colabore e evolua sua arquitetura de software com diagramas dinâmicos e sempre atualizados, gerados a partir do seu código.

[Documentação](https://likec4.dev/pt-br/) - [Playground](https://playground.likec4.dev/) - [Demonstração](https://template.likec4.dev/view/index)

<a href="https://www.npmjs.com/package/likec4" target="_blank"> ![Versão no NPM](https://img.shields.io/npm/v/likec4) </a>
<a href="https://www.npmjs.com/package/likec4" target="_blank">![Downloads no NPM](https://img.shields.io/npm/dm/likec4)</a>
<a href="https://marketplace.visualstudio.com/items?itemName=likec4.likec4-vscode" target="_blank">![Instalações no VSCode](https://img.shields.io/visual-studio-marketplace/azure-devops/installs/total/likec4.likec4-vscode?label=vscode%20installs)</a>
<a href="https://open-vsx.org/extension/likec4/likec4-vscode" target="_blank">![Downloads no Open VSX](https://img.shields.io/open-vsx/dt/likec4/likec4-vscode?label=open-vsx&color=%23A60EE5)</a>
<a href="https://mcptoplist.com/server/mcp.so%2FLikeC4%2FLikeC4" target="_blank">![MCP Toplist](https://mcptoplist.com/badge/mcp.so%2FLikeC4%2FLikeC4.svg)</a>

![Extensão do VSCode](https://github.com/likec4/likec4/assets/824903/d6994540-55d1-4167-b66b-45056754cc29)

## O que é o LikeC4? Por que "Like"?

O LikeC4 é uma linguagem de modelagem para descrever arquiteturas de software e um conjunto de ferramentas para gerar diagramas a partir do modelo.

O LikeC4 é inspirado no [C4 Model](https://c4model.com/) e no [Structurizr DSL](https://github.com/structurizr/dsl), mas oferece flexibilidade adicional.
Você pode personalizar ou definir sua própria notação, tipos de elementos e qualquer quantidade de níveis aninhados no modelo de arquitetura.\
Tudo adaptado às suas necessidades.

## Como é o LikeC4 na prática?

Código-fonte em LikeC4:

<div align="center">
  <img src="https://github.com/likec4/.github/assets/824903/c0f22106-dba6-469e-ab47-85e7b8565513" width="675px">
</div>

Execute a [CLI](./packages/likec4/README.md) para visualizar:

```sh
npx likec4 start
```

Resultado:

<div align="center">
  <img src="https://github.com/likec4/likec4/assets/824903/27eabe54-7d97-47a8-a7e4-1bb44a8e03e5" width="984px">
</div>

Repositório de template — [likec4/template](https://github.com/likec4/template)\
Disponível em — [https://template.likec4.dev](https://template.likec4.dev/view/index)

[![Abrir no StackBlitz](https://developer.stackblitz.com/img/open_in_stackblitz.svg)](https://stackblitz.com/~/github.com/likec4/template)

Confira o [Tutorial](https://likec4.dev/pt-br/tutorial/) para ter uma visão geral rápida do LikeC4.

## Como obter ajuda

Estamos sempre disponíveis para ajudar você a começar:

- [Participe da comunidade no Discord](https://discord.gg/86ZSpjKAdA) — é a forma mais fácil de obter ajuda
- [GitHub Discussions](https://github.com/likec4/likec4/discussions) — tire dúvidas sobre o projeto ou envie seu feedback

## Colaboradores

<a href="https://github.com/likec4/likec4/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=likec4/likec4" />
</a>

[Torne-se um colaborador](./CONTRIBUTING.md)

## Apoie o desenvolvimento

O LikeC4 é um projeto de código aberto sob a licença MIT, e seu desenvolvimento contínuo só é possível graças ao seu apoio.\
Se você gosta do projeto, considere contribuir financeiramente para ajudar no seu crescimento e aprimoramento.\
Você pode apoiar o projeto pelo [OpenCollective](https://opencollective.com/likec4) ou pelo [GitHub Sponsors](https://github.com/sponsors/likec4).

## Licença

Este projeto é disponibilizado sob a [Licença MIT](LICENSE).

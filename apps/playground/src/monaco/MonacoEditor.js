...
const LazyMonacoEditor = memo((props) => {
    const { setLayoutedModelApi } = props;
    const playground = usePlayground();
    const onActiveEditorChanged = useCallbackRef((filename) => {
    ...
    const onLoad = useCallbackRef((wrapper) => {
    ...
    return (<>
      <MonacoEditorReactComp style={{ width: '100%', height: '100%' }} wrapperConfig={wrapperConfig} onLoad={onLoad} onError={useCallbackRef(err => {
            ...
            if (import.meta.env.DEV && isConnectionError) {
                console.warn('[LikeC4] Language server connection failed. Check the console for "[LikeC4 LSP worker]" for the cause. Try: refresh the page; run `pnpm build` in the repo root and restart the dev server.');
            }
        })}/>
      {wrapper && (<LanguageClientSync config={wrapperConfig} wrapper={wrapper} {...(setLayoutedModelApi != null && { setLayoutedModelApi })}/>)}
    </>);
});

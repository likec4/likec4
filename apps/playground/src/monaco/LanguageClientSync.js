...
import { BuildDocuments, ChangeView, DidChangeModelNotification, FetchComputedModel, FetchLayoutedModel, LayoutView, Locate, } from '@likec4/language-server/protocol';
...
export function LanguageClientSync({ config, wrapper, setLayoutedModelApi, }) {
    const router = useRouter();
    const playground = usePlayground();
    const { workspaceId, activeFilename, playgroundState, activeViewId, activeViewState, } = usePlaygroundSnapshot(s => ({
    ...
    const languageClient = () => nonNullable(wrapper.getLanguageClient('likec4'), 'MonacoEditorLanguageClientWrapper is missing LikeC4 LanguageClient');
    ...
    const registerLayoutedModelApi = useCallbackRef(() => {
        if (!setLayoutedModelApi || !wrapper)
            return;
    ...
    const requestLayoutView = useCallbackRef(async (viewId) => {
        try {
    ...
    const revealLocation = (location) => {
        const editor = wrapper.getEditor();
        const model = monaco.editor.getModel(monaco.Uri.parse(location.uri));
        if (editor && model) {
            const range = new monaco.Range(location.range.start.line + 1, location.range.start.character + 1, location.range.end.line + 1, location.range.end.character + 1);
    ...
    const showLocation = useCallbackRef(async (target) => {
        try {
    ...
    const applyViewChanges = useCallbackRef(async (viewId, change) => {
        try {
    ...
    useEffect(() => {
        ...
        lastAsync.current = lastAsync.current
            ...
            const { docs, activeModel } = createMemoryFileSystem(config.fsProvider, ctx.files, ctx.activeFilename);
            if (activeModel) {
            ...
            disposables.push(languageClient().onNotification(DidChangeModelNotification.type, () => {
                try {
                    const errors = [];
                    languageClient().diagnostics?.forEach((uri, diagnostics) => {
                        for (const diagnostic of diagnostics) {
                            if (diagnostic.severity === 0) {
                                errors.push(`L${diagnostic.range.start.line + 1}:${diagnostic.range.start.character + 1} ${diagnostic.message}`);
                            }
                        }
                    });
                    playground.send({
    ...
    useEffect(() => {
        if (playgroundState !== 'ready' || activeViewId == null)
            return;

...
export function WorkspaceFileTabs() {
    ...
    return (<HStack className={css({})} justify={'space-between'}>
      <Tabs style={{
            overflowX: 'scroll',
        }} value={activeFilename} onChange={v => {
            playground.changeActiveFile(v ?? playground.getActiveFile().filename);
        }}>
        <TabsList style={{
            flexWrap: 'nowrap',
        }}>
          {filenames.map(filename => (<Tabs.Tab key={filename} value={filename} fz={'xs'} fw={'500'}>
              {filename}
            </Tabs.Tab>))}
          {
        /* <Button
      ...
      {diagnosticErrors.length > 0 && (<HoverCard position="right-start">
          <HoverCard.Target>

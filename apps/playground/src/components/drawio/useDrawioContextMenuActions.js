...
import { buildDrawioExportOptionsForViews, buildDrawioExportOptionsFromSource, DEFAULT_DRAWIO_ALL_FILENAME, generateDrawio, generateDrawioMulti, } from '@likec4/generators';
...
function downloadDrawioBlob(xml, filename) {
...
function toViewModel(view, styles) {
    return {
        $view: view,
        get $styles() {
...
function reportExportError(message, err, onExportError) {
    if (onExportError)
        onExportError(message, err);
    else
        console.error(message, err);
}
/** Phase 1: fill byId from layouted model (LSP). */
async function fillFromLayoutedModel(byId, getLayoutedModel, styles, onExportError) {
    try {
...
function fillFromViewStates(byId, viewIdsInModel, viewStates, styles) {
...
async function fillFromLayoutViews(byId, missing, layoutViews, styles, onExportError) {
    try {
        const diagrams = await layoutViews(missing);
        for (const viewId of missing) {
            const diagram = diagrams[viewId];
            if (diagram)
                byId.set(viewId, toViewModel(diagram, styles));
        }
    }
    catch (e) {
...
function fillFromModelView(byId, viewIdsInModel, likec4model, styles) {
    for (const viewId of viewIdsInModel) {
        if (!byId.has(viewId)) {
            try {
                // Casts bridge generic model API (ViewId) and LayoutedView to concrete DiagramView for DrawIO export
                const vm = likec4model.view(viewId);
                if (vm?.$view)
                    byId.set(viewId, toViewModel(vm.$view, styles));
            }
            catch {
...
async function collectViewModelsForExportAll(options) {
    const { viewIdsInModel, allViewModelsFromState, likec4model, viewStates, getLayoutedModel, layoutViews, onExportError, } = options;
    if (!likec4model || viewIdsInModel.length === 0)
        return [];
    const styles = likec4model.$styles ?? null;
    const byId = new Map();
    for (const vm of allViewModelsFromState)
        byId.set(vm.$view.id, vm);
    if (getLayoutedModel)
        await fillFromLayoutedModel(byId, getLayoutedModel, styles, onExportError);
    ...
    return viewIdsInModel.map(id => byId.get(id)).filter(Boolean);
}
/**
 ...
 */
export function useDrawioContextMenuActions({ diagram, likec4model, viewStates = {}, getSourceContent, getLayoutedModel, layoutViews, onExportError, }) {
    const allViewModelsFromState = useMemo(() => {
        if (!likec4model)
            return [];
        const styles = likec4model.$styles;
        return Object.values(viewStates)
            .filter((vs) => vs?.state === 'success' && !!vs.diagram)

...
function CurrentUser({ session }) {
    return (<Menu shadow="md">
      <MenuTarget>
        <Button variant="subtle" px={'xs'} size="xs" color="gray" leftSection={<Avatar src={session.avatarUrl} size={18}>
              {session.login}
            </Avatar>}>
          @{session.login}
        </Button>
...
export function UserButton() {
    ...
    return (<Button loading={loading} variant="default" component="a" href="/auth/github" color="gray" onClick={() => {
            setLoading(true);

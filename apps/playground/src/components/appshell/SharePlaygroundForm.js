...
import { Alert as MantineAlert, Anchor, Box, Button as MantineButton, Checkbox, CopyButton, Group, Input, InputLabel, PinInput, Select, Stack, Text, UnstyledButton, } from '@mantine/core';
...
export function SharePlaygroundForm() {
    ...
    return (<>
      ...
      <div className={hstack({ gap: 'sm', alignItems: 'baseline' })}>
        <Select variant="filled" size="xs" label="Link expires after" lh={'xs'} value={expires} onChange={c => ShareOptions.isValidExpires(c) && setExpires(c)} allowDeselect={false} checkIconPosition="right" w={150} comboboxProps={{
            offset: 2,
            withinPortal: false,
        }} data={ShareOptions.ExpiresValues}/>
        <Stack gap={'xs'} align="flex-start">
          <Select variant="filled" size="xs" label="Allow Access" lh={'xs'} checkIconPosition="right" allowDeselect={false} value={access} onChange={c => ShareOptions.isValidAccess(c) && setAccess(c)} comboboxProps={{
            offset: 2,
            withinPortal: false,
          ...
          {access === 'pincode' && (<Box>
              <Group justify="space-between">
                ...
                _hover: {
                  ...
                  <UnstyledButton disabled={pincode.length < 4} className={css({
                fontSize: 'x-small',
                color: 'mantine.colors.dimmed',
                transition: 'all',
                _hover: {
              ...
              <PinInput size="xs" value={pincode} onChange={setPincode} gap="sm" error={pincode.length < 4} placeholder=""/>
            </Box>)}
        </Stack>
      </div>
      <div>
        <Checkbox size="xs" checked={forkable} onChange={e => setForkable(e.currentTarget.checked)} label="Allow copying" description="Allow others to fork this playground"/>
      </div>
      {shareRequest?.error && <Alert title={shareRequest.error}/>}
      {!shareLink &&
            (<>
            {access.startsWith('github:') && (<HStack w={'100%'}>
                <Alert color="main" w={'100%'}>
                  <Text className={css({
                        fontSize: 'xs',
                        // color: 'main.8',
                    })}>
                    <Anchor underline="not-hover" href="mailto:denis@davydkov.com?subject=Feature%3A%20allow%20access%20via%20GitHub" className={css({
                        fontSize: 'xs',
                        color: 'text',
                    })}>
                      Contact
            ...
            <Group justify="space-between" align="baseline" w={'100%'}>
              ...
              <Button disabled={btnDisabled} loading={isShareInProgress} size="xs" onClick={(e) => {
                    e.stopPropagation();
                    playground.send({
      ...
      {shareRequest?.success && shareLink && (<CopyButton value={shareLink} timeout={2000}>
          {({ copied, copy }) => (<Alert title="Share link" color="green" w={'100%'}>
              <Stack gap={'2xs'}>
                <Input value={shareLink} variant="unstyled" readOnly size="xs" c="green" px={0} classNames={{
                    input: css({
                        cursor: 'pointer',
                ...
                <div className={hstack({ gap: 'xs', alignItems: 'baseline' })}>
                  <Button onClick={copy} color="green" variant={copied ? 'light' : 'subtle'} size="compact-xs" fz="2xs" px="2xs" c="green" fw="normal">

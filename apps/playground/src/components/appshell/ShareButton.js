...
import { Alert as MantineAlert, Button as MantineButton, Popover, Stack, Text, } from '@mantine/core';
...
export function ShareButton() {
    const { diagnosticErrors } = usePlaygroundSnapshot(s => ({
        diagnosticErrors: s.context.diagnosticErrors,
    }));
    ...
    const [, { createNewFromCurrent, }] = useWorkspaces();
    if (isExample) {
        return (<Popover opened={examplePopoverOpened} withOverlay overlayProps={{
                opacity: 0,
            }} position="bottom-start" withArrow arrowPosition="center" onDismiss={() => setExamplePopoverOpened(false)} shadow="md" //
        >
        <Popover.Target>
          <Button color="gray" variant="subtle" leftSection={<IconShare size={14}/>} onClick={() => setExamplePopoverOpened(true)}>
            Share
          </Button>
        </Popover.Target>
    ...
    return (<Popover withOverlay position="bottom-start" withArrow arrowPosition="center" shadow="md" keepMounted={false}>
      <Popover.Target>

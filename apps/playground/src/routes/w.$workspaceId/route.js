...
function WorkspaceContextPage() {
    ...
    const [layoutedModelApi, setLayoutedModelApi] = useState(null);
    const { defaultLayout, onLayoutChange } = useDefaultLayout({
        groupId: 'likec4-playground',
        storage: sessionStorage,
    });
    return (<PlaygroundActorContextProvider workspace={workspace}>
      <DrawioContextMenuProvider layoutedModelApi={layoutedModelApi}>
        <AppShell header={{ height: 50 }}>
          ...
          <AppShellMain h={'100dvh'}>
            <Group className={css({ h: '100%' })} orientation={isMobile ? 'vertical' : 'horizontal'} defaultLayout={defaultLayout} onLayoutChange={onLayoutChange}>
              <Panel id="editor" className={styles.panel} collapsible={true} minSize={'10'} defaultSize={'60'}>
                <Stack h="100%" gap={0}>
                  <WorkspaceFileTabs />
                  <EditorPanelWithDrawioMenu setLayoutedModelApi={setLayoutedModelApi}/>
                </Stack>
              </Panel>
              <Separator className={styles.resize} style={{
...
function EditorPanelWithDrawioMenu({ setLayoutedModelApi, }) {
    const { openMenu } = useDrawioContextMenu();
    return (<Box flex={1} onContextMenu={e => {
            e.preventDefault();
            openMenu(e);
        }} style={{ minHeight: 0 }}>

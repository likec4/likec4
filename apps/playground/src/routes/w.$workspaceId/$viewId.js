...
function WorkspaceDiagramPage() {
    ...
    const viewId = Route.useParams().viewId;
    const playground = usePlayground();
    const { workspaceId, playgroundState, likec4model, diagram, state, error, } = usePlaygroundSnapshot(c => {
    ...
    if (_diagram && likec4model) {
        ...
        <LikeC4PlaygroundEditor>
          <LikeC4ModelProvider likec4model={likec4model}>
            <LikeC4Diagram view={_diagram} controls fitView fitViewPadding={{
                top: '70px',
                bottom: '32px',
                left: '60px',
                right: '32px',
            }} nodesSelectable showNavigationButtons enableElementDetails enableDynamicViewWalkthrough enableRelationshipBrowser enableRelationshipDetails enableNotations enableElementTags enableFocusMode enableSearch enableCompareWithLatest={false} reactFlowProps={{
                // Otherwise reactflow intercepts "Space" key
                panActivationKeyCode: null,
            }} renderIcon={IconRenderer} onCanvasContextMenu={onCanvasContextMenu} onNavigateTo={(nextView, event) => {
            ...
            {error && (<Box className={css.stateAlert}>
                <Notification icon={<IconX style={{ width: 20, height: 20 }}/>} color="red" title="Error" withCloseButton={false}>
                  {error}
                </Notification>
              </Box>)}
          </LikeC4ModelProvider>
    ...
    return (<Box pos={'relative'} w={'100%'} h={'100%'}>
      ...
      <Box className={css.stateAlert}>
        <Notification icon={icon} loading={state === 'pending'} color={isInvalid ? 'red' : 'teal'} withCloseButton={false}>
          {error ?? 'Loading...'}
        </Notification>
      </Box>
    </Box>);
}
function LikeC4PlaygroundEditor({ children }) {
    const actor = usePlaygroundActorRef();
    return (<LikeC4EditorProvider editor={{
            fetchView: () => {

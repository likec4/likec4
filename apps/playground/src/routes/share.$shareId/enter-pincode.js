...
import { Button, Center, Container, Paper, PinInput, Stack, Text, Title, } from '@mantine/core';
...
function RouteComponent() {
    ...
    const { value: pincode = '', set: setPincode, } = useLocalStorageValue('last-entered-pincode', {
        defaultValue: '',
    });
        ...
        <Stack>
          {pincodeValid
            ? (<>
                ...
                <Center mt={'md'}>
                  <IconRosetteDiscountCheck className={css({
            ...
            : (<form onSubmit={async (e) => {
                ...
                <Center mt={'md'}>
                  <PinInput size="lg" autoFocus value={pincode} onChange={setPincode} onComplete={value => value.substring(0, 3) === prev?.substring(0, 3) && checkPin.execute()} disabled={data.status === 'loading'}/>
                </Center>
                <Center mt={'xl'}>
                  <Button type="submit" loading={data.status === 'loading'} disabled={pincode.trim().length < 4}>
                    Open
                  </Button>
                </Center>
              </form>)}

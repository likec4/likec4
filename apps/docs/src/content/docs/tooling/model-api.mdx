...
`likec4.toBuilder(project?)` returns a [Model Builder](#model-builder) pre-populated with the loaded
workspace. You can then chain `.model(...)`, `.deployment(...)` and `.views(...)` to add new content,
and call `.toLikeC4Model()` (or `.build()`) to get the enriched model.
...
### `Builder.allowOverwrites()` — edit seeded elements

By default the Builder is strict: re-declaring an FQN that is already present throws. Opt in with
`.allowOverwrites()` when you want to *edit* seeded entries (for example, to update a title or merge in
new children):
...
In overwrite mode, same-FQN + same-kind redeclaration replaces the element's properties. Redeclaring
with a *different kind* still throws (it's almost certainly a programmer error).
...
:::caution
**Type safety when loading from disk.** When the parsed model comes from a workspace, the element kinds
and FQNs are only known at runtime — so the builder returned by `LikeC4.toBuilder()` has loose types.
Callers who know the spec statically can supply it explicitly:
...
Cross-references to existing FQNs (e.g. `rel('cloud.api', 'cloud.ui')`) can't be statically validated
either way — they're checked at builder-mutation time.
:::

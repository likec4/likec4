...
| `render-view` | Renders an existing saved view as an interactive diagram (pan/zoom/fit) inline in chat, for hosts that support [MCP Apps](https://github.com/modelcontextprotocol/ext-apps) |
| `preview-view` | Renders a preview of a LikeC4 view from DSL text without saving files, useful for drafting and iterating in chat |

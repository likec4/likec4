...
<Aside type="note" title="Experimental: Enhanced Sequence Layouts">
  Upcoming enhancements to sequence diagram layouts will support branch collections (parallel and alternate paths) with improved visual representation. When the feature becomes available, layouted dynamic views will include branch metadata such as `branchId`, `pathId`, and `kind` for each branch region. This enables richer interactive experiences for walkthrough navigation across different execution paths.

  **Current Status**: Foundation types and layout engine updates are in progress. Branch visualizations and UI controls will be added in future releases.
</Aside>

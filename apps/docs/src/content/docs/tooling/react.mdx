...
<Tabs>
 <Tab label='Model from builder'>    
  ...
  <Tab label='From dump'>
    Prepare model with [export](/tooling/cli/#export-to-json):

    ```sh
    likec4 export json -o ./src/likec4-dump.json
    ```
    ...
    import { LikeC4ModelProvider } from '@likec4/diagram/bundle'
    import dump from './likec4-dump.json' with { type: 'json' }

    const likec4model = LikeC4Model.fromDump(dump)
...
If you use built-in icons, install [`@likec4/icons`](https://www.npmjs.com/package/%40likec4%2Ficons) and:
...
function LikeC4View({viewId}: {viewId: string}) {
  const model = useLikeC4Model(true).findView(viewId)
  if (!model?.isDiagram()) {
...
You can render any component inside `LikeC4Diagram` (or `LikeC4View`/`ReactLikeC4` if you are using bundle):

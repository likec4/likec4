...
description: Learn how to use LikeC4 Vite Plugin to embed your architecture diagrams.
head:
  - tag: title
...
LikeC4 Vite Plugin allows you to embed views from your model into Vite-based application.  
The plugin will automatically generate all the necessary code to render the views in your application, with Hot Module Replacement (HMR) supported.
...
For example, export everything from `likec4:react`:
...
If you run `vite build --watch` as a background process, you’ll even get auto-updates on any change (though probably without HMR).
...
- Check [React code generation](/tooling/code-generation/react/)   

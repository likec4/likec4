...
The generator accepts optional **options** for round-trip or tooling: `layoutOverride` (per-node `{ x, y, width, height }`), `strokeColorByNodeId`, and `strokeWidthByNodeId`. When present, element/edge **customData** is written as mxUserObject, and edge **waypoints** (viewmodel `points`) are written as mxGeometry with an Array of mxPoints.

...
As in the logical model, you can [`extend`](/dsl/workspace/#extend-element) deployment nodes:
...
Same [rules](/dsl/workspace/#extend-element) apply for extending nodes:
- extended node must be referenced by a fully qualified name.  
- define [additional properties](/dsl/workspace/#additional-properties).

...
- edge routing: optional, possible values are `spline` or `ortho`. See [Edge routing](#edge-routing).
...
In the web editor, switch routing from the layout panel; the change is written back to the view source.

...
<Aside type="caution">
Orthogonal routing is done by Graphviz and has a known limitation: a route may occasionally cut through a group box it does not belong to.  
Widen the view with the `autoLayout` spacing parameters, or adjust the connection by editing its control points.
</Aside>

...
- edge routing: optional, `spline` or `ortho` (see [Edge routing](#edge-routing))
...
By default, relationships are drawn as curves.  
Set `routing ortho` to draw them with horizontal and vertical segments and right-angle bends:
...
Possible values are `spline` (default, curved) and `ortho` (orthogonal).  
The value can also be appended to `autoLayout`, for example `autoLayout LeftRight 120 110 ortho`.  
Precedence: the view `routing` property, then `autoLayout`, then the project default [`styles.defaults.view.routing`](/dsl/config/#default-styles), then `spline`.

Routing also applies to manually laid-out views: node positions and edited connections are kept and drawn with right angles,
so switching a view or a whole project to `ortho` never invalidates a saved layout.

<Aside type="caution">
Graphviz's orthogonal router can draw a route through a group box it does not belong to, or place parallel routes so close together that they are hard to distinguish.
Increasing the `autoLayout` spacing can give nodes more room, but does not guarantee more space between routes.

For a crowded view, show fewer relationships or adjust individual connections by editing their control points.
Edited edges that share a straight run move onto separate tracks where space permits. Untouched Graphviz routes keep their original positions, so they can remain closely packed.
</Aside>

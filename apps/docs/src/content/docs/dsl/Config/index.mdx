...
## Relationship browser scope

Configure the initial scope in the relationship browser.
The allowed values are `global` and `view`. The default is `view`.
...
A saved user preference takes precedence over `defaultScope`.
The `relationshipsScope` URL parameter takes precedence over both the saved preference and `defaultScope`.
...
To set project defaults for elements, relationships, and views, add a `styles.defaults` section to your configuration file. Explicit style properties override these defaults.

For example, this configuration sets default element and relationship styles and enables orthogonal routing for views:
...
```jsonc
{
  ...
  "styles": {
    "defaults": {
      ...
      // Defaults for views
      "view": {
        // Edge routing: "spline" (curved, default) or "ortho" (right-angle bends)
        // Override this default with routing or autoLayout in a view.
        "routing": "ortho"
      }
    }
  }
...
A view's `routing` property or `autoLayout` routing parameter overrides `styles.defaults.view.routing`. See [Edge routing](/dsl/views/predicates/#edge-routing) for precedence and limitations.

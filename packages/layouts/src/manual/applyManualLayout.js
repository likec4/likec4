...
export function applyManualLayout(diagramView, manualLayout) {
    ...
    // Place new nodes
    const previousBb = BBox.merge(...diagramView.nodes
        .map(node => manualLayout.nodes[node.id])
        .filter(n => !!n));
    diagramView.nodes
        .filter(node => node.parent == null)
        .reduce((acc, node) => layoutNode(node, acc, nodes, manualLayout), { x: previousBb.x, y: previousBb.y + previousBb.height + rankSep });
    // Add new edges
    diagramView.edges
...
function layoutNode(node, basePoint, nodes, manualLayout) {
    ...
    node.children
        ...
        .reduce((acc, child) => layoutNode(child, acc, nodes, manualLayout), placeChildrenAt);
    // Layout node itself
...
function canApplySafely(diagramView, manualLayout) {
    const isCompound = (node) => node.children.length > 0;

...
export function estimateNodeSize(node: ComputedNode) {
  ...
  const descriptionWidth = node.description ? textPixelWidth(node.description, {
    font: 'helvetica',
  ...
  const descriptionHeight = descriptionLines === 0 ? 0 : Math.ceil(descriptionLines * sizes.description.lineHeight * sizes.description.fontSize)

  const height = Math.max(
    sizes.height,
    titleHeight + padding + (descriptionHeight > 0 ? descriptionHeight + sizes.padding - sizes.title.fontSize : 0),
  )

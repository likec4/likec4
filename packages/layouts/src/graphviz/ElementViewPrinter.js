...
export class ElementViewPrinter extends DotPrinter {
    postBuild(G) {
        this.assignGroups();
        ...
        for (const compound of compounds) {
            const children = pipe(compound.children, filter(id => !this.compoundIds.has(id)), map(id => this.computedNode(id)), filter(nd => nd.inEdges.length === 0 && nd.outEdges.length === 0));
        ...
        if (explicitRankBlocks > 0) {
            G.set('likec4_rankBlocks', explicitRankBlocks);
        }
    }
    applyExplicitRankBlocks(G) {

...
export function toGraphvisModel({
  ...
  function resolveEdgeEndpoint(
    ...
    return [computedNode, endpoint, ltail] as const
  }
  ...
  function addEdge<E extends ComputedEdge>(edge: E, parent: GraphBaseModel) {
    ...
    const [sourceNode, source, ltail] = resolveEdgeEndpoint(sourceFqn, nodes => last(nodes))
    const [targetNode, target, lhead] = resolveEdgeEndpoint(targetFqn, first)
...
export function printElementViewToDot(view: ComputedView): DotSource {

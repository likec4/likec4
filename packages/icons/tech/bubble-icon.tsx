// @ts-nocheck

import type { SVGProps } from 'react'
const SvgBubbleIcon = (props: SVGProps<SVGSVGElement>) => (
  <svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid" viewBox="0 0 256 276" {...props}>
    <path
      fill="#262626"
      d="M156.688 71.53c-24.76 0-49.164 10.63-67.795 31.529V0H52.024v173.519c0 56.326 45.661 101.988 101.988 101.988S256 229.845 256 173.519 213.014 71.531 156.688 71.531m-2.676 164.694c-34.632 0-62.707-28.075-62.707-62.706s28.075-62.707 62.707-62.707c34.631 0 62.706 28.075 62.706 62.707s-28.075 62.706-62.706 62.706"
    />
    <path
      fill="#00F"
      d="M25.22 225.066C11.29 225.066 0 236.358 0 250.286s11.291 25.22 25.22 25.22 25.22-11.292 25.22-25.22-11.291-25.22-25.22-25.22"
    />
  </svg>
)
export default SvgBubbleIcon

import { BorderStyles, computeColorValues, ElementShapes, IconPositions, RelationshipArrowTypes, Sizes, ThemeColors, } from '@likec4/core/styles';
...
const StrictThemeColorValuesSchema = z.strictObject({
    elements: ElementColorValuesSchema
        .or(colorSchema.transform(v => computeColorValues(v).elements))
        .meta({
    ...
    relationships: RelationshipColorValuesSchema
        .or(colorSchema.transform(v => computeColorValues(v).relationships))
...
export const ThemeColorValuesSchema = StrictThemeColorValuesSchema.or(colorSchema.transform(v => computeColorValues(v)))
    .transform(value => value)
    .meta({
    id: 'ThemeColorValues',
    description: 'Exact value (hex, rgb, rgba, hsl, hsla ...) or break down of specific color values',
});
const ThemeColorsSchema = z.partialRecord(color, ThemeColorValuesSchema)
    .transform(value => value);
...
export const LikeC4Config_Styles_Theme = z
    ...
    return exact({
        colors: colors ? exact(colors) : undefined,
        sizes: sizes ? exact(sizes) : undefined,
    });
});
...
const LikeC4Config_Styles_CustomStylesheets = z
    ...
    z.array(z.string().min(1, 'Custom CSS file path cannot be empty')),
])
    .meta({

...
import type { AnyAux, ComputedLikeC4ModelData, ComputedView, LayoutedLikeC4ModelData, LayoutedView, ParsedLikeC4ModelData, scalar, ViewType, ViewWithType } from '../types';
...
export type AnyScalar<V extends string> = Tagged<V, 'Fqn'> | Tagged<V, 'DeploymentFqn'> | Tagged<V, 'ViewId'> | Tagged<V, 'NodeId'> | Tagged<V, 'EdgeId'> | Tagged<V, 'RelationId'>;
export type ElementOrFqn<A> = aux.ElementId<A> | {
    id: aux.Fqn<A>;
};
export type DeploymentOrFqn<A> = aux.DeploymentId<A> | {
    id: aux.DeploymentFqn<A>;
};
export type ViewOrId<A> = aux.ViewId<A> | {
    id: aux.StrictViewId<A>;
};
export type NodeOrId = string | {
    id: scalar.NodeId;
};
export type EdgeOrId = string | {
    id: scalar.EdgeId;
};
export type RelationOrId = string | {

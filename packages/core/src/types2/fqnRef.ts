...
export namespace FqnRef {
  ...
  export function isElementRef<A extends AnyAux>(ref: FqnRef<A>): ref is ElementRef<A> {
    return 'fqn' in ref && !('project' in ref)
  }
  ...
  export function isImportRef<A extends AnyAux>(ref: FqnRef<A>): ref is ImportRef<A> {
    return 'project' in ref && 'fqn' in ref
  }

  export function flatten<A extends AnyAux>(ref: FqnRef<A>): Aux.Strict.Fqn<A> {
    if (isImportRef(ref)) {
      return GlobalFqn(ref.project, ref.fqn)
    }
    if (isElementRef(ref)) {
      return ref.fqn as unknown as Aux.Strict.Fqn<A>
    }
...
// type TestAux = Aux<'project', 'element', 'deployment', 'view'>

// const isFqnRef: FqnRef<TestAux> = {
//   fqn: 'element',
// }

// if (FqnRef.isModelRef(isFqnRef)) {
//   isFqnRef.fqn
// }

...
function testCalcDrifts<const Nodes, Edges>(patches?: Patches<Nodes, Edges>) {
  const {
    snapshot,
    layouted,
    layoutedNodes,
  ...
  const nodes = indexBy(result.nodes, n => n.id) as typeof layoutedNodes
  const edges = indexBy(result.edges, e => e.id) as typeof layoutedEdges

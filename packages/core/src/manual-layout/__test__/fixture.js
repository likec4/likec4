...
function simplebuilder() {
    return Builder
        ...
        .views(({ view, $include }, _) => _(view('index', {
        title: 'Landscape',
        description: 'System Landscape',
    }).with($include('*'), $include('saas.*'))));
}
 ...
 * })
 */
export function prepareFixtures(patcher) {
    const snapshot = structuredClone(indexSnapshot);
    let layouted = {
        ...patch(structuredClone(snapshot), patcher?.view),
    };
    if (patcher?.nodes) {
        const patchNodes = {
            ...patcher.nodes,
        };
        const writableNodes = layouted.nodes = pipe(layouted.nodes, filter(n => !(n.id in patchNodes) || isTruthy(patchNodes[n.id])), map(n => {
    ...
    if (patcher?.edges) {
        ...
        const writableEdges = layouted.edges = pipe(layouted.edges, filter(n => !(n.id in patchEdges) || isTruthy(patchEdges[n.id])), map(n => {
            const edgePatcher = patchEdges[n.id];
            if (edgePatcher) {
                delete patchEdges[n.id];
        ...
        for (const [id, patcher] of Object.entries(patchEdges)) {
            if (isTruthy(patcher)) {
                ...
                writableEdges.push(patch(baseEdge, patcher));
            }
        }
    }
    return {
        ...
        layouted,
        layoutedNodes: indexBy(layouted.nodes, n => n.id),
        layoutedEdges: indexBy(layouted.edges, e => e.id),
    };
}

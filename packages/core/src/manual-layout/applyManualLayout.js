...
import { _layout, isDynamicView, isElementView, } from '../types';
...
function autoApplyLabelsToLeaf(draft, next, sizeNotChanged) {
    ...
    if (changed(draft.description, next.description)) {
        if (isNullish(next.description)) {
            ...
            draft.description = patchMarkdownOrString(draft.description, next.description);
        }
        else {
            applied = false;
        }
 ...
 */
export function applyManualLayout(autoLayouted, snapshot) {
        ...
        return produce(node, draft => {
            ...
            if (changed(draft.notes, next.notes)) {
                if (isNullish(next.notes)) {
                    ...
                    draft.notes = patchMarkdownOrString(draft.notes, next.notes);
                }
            }
            if (changed(node.notation, next.notation)) {
                draft.notation = next.notation ?? null;
            }
            if (wasCompound && willBeCompound
...
function applyEdgesManualLayout(snapshotEdges, latestEdges, viewDrifts) {
    ...
    const edges = snapshotEdges.map((edge) => {
        let next = nextEdges.get(edge.id) ?? pipe(nextEdges.values(), ifilter(e => e.source === edge.source && e.target === edge.target), ihead());
        if (next) {
                ...
                switch (true) {
                    ...
                    case !!edge.labelBBox && !!next.labelBBox: {
                        ...
                        if (changed(edge.description, next.description)) {
                            ...
                            if (next.description) {
                                draft.description = patchMarkdownOrString(edge.description, next.description);
                            }
                        }

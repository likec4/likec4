...
import type { AnyAux, Aux, BorderStyle, ElementShape, ElementSpecification as ElementKindSpecification, Expression, IconPosition, KeysOf, LikeC4ProjectStylesConfig, MarkdownOrString, NonEmptyArray, RelationshipArrowType, RelationshipLineType, RelationshipSpecification as RelationshipKindSpecification, ShapeSize, SpacingSize, SpecAux, TagSpecification, TextSize, ThemeColor as Color } from '../types';
...
export type Metadata<MetadataKey extends string> = IsNever<MetadataKey> extends true ? never : IsLiteral<MetadataKey> extends true ? {
...
export type Warn<Id, Existing> = IsLiteral<Existing> extends true ? Id extends Existing ? Invalid<'Already exists'> : Id : Id;
export type ValidId<Id> = Id extends `${number}${string}` ? Invalid<'Id must not start with number'> : IsLiteral<Id> extends true ? Id : Invalid<'Id must be a literal'>;
/**
 * Auxilary type to keep track of the types in builder
 */
export interface Types<ElementKind extends string, Fqn extends string, ViewId extends string, RelationshipKind extends string, Tag extends string, MetadataKey extends string, DeploymentKind extends string, DeploymentFqn extends string> {
...
export declare namespace Types {
    ...
    type ToAux<T> = T extends AnyTypes ? Aux<'parsed', T['Fqn'], T['DeploymentFqn'], T['ViewId'], 'from-builder', SpecAux<T['ElementKind'], T['DeploymentKind'], T['RelationshipKind'], T['Tag'], T['MetadataKey']>> : AnyAux;

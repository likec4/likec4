...
import { isViewRuleStyle } from '../types';
...
describe('Builder (style 2)', () => {
    ...
    describe('general', () => {
        const b = spec
            .model(({ system, actor, component, rel }, _) => _(actor('customer'), system('cloud').with(component('ui'), component('backend')), rel('customer', 'cloud.ui', {
            kind: 'like',
        })))
            .deployment(({ env, node, instanceOf }, _) => _(env('prod').with(node('eu').with(instanceOf('cloud.ui'))), env('dev'), node('dev.local')))
            .views(({ view, viewOf, deploymentView, $include }, _) => _(view('index', 'Index').with($include('cloud.*')), viewOf('cloud', 'cloud.ui').with($include('* -> cloud.**')), deploymentView('deployment', 'Deployment').with($include('prod.**'))));
    ...
    it('should build with string-based spec', async ({ expect }) => {
        const m = Builder
            ...
            .model(({ _, actor, system, component, rel }) => _(actor('customer', { tags: ['tag1'] }), system('cloud', { tags: ['tag2'] }), component('cloud.ui'), rel('customer', 'cloud.ui', {
            kind: 'like',
            tags: ['tag1'],
        })))
            .deployment(({ _, env, node, instanceOf }) => _(env('prod').with(node('eu').with(instanceOf('cloud.ui')))))
            .toLikeC4Model();
    ...
    it('should fail if invalid ID provided ', () => {
        expect(() => {
            spec.model(({ actor }, _) => _(actor('cust.omer')));
        }).toThrowError('Parent element with id "cust" not found for element with id "cust.omer"');
    });
    it('should fail on invalid instance ', () => {
        const b = spec
            .model(_ => _.model(_.component('cloud'), _.component('cloud.ui')));
        expect(() => {
            b.deployment(_ => _.deployment(_.instanceOf('cloud.ui'))).build();
        }).toThrowError('Instance ui of cloud.ui must be deployed under a parent node');
        // Nested instanceOf is correct
        const raw = b.deployment(_ => _.deployment(_.node('node').with(_.instanceOf('cloud.ui')))).build();
    ...
    it('should build and compute LikeC4Model', async ({ expect }) => {
        const m = spec
            .model(({ system, actor, component, rel }, _) => _(actor('customer'), system('cloud').with(component('ui'), component('api')), rel('customer', 'cloud.ui', {
            title: 'uses',
            tags: ['tag1'],
            ...
            .views(({ view, viewOf, $include }, _) => _(view('index', 'Index').with($include('*')), viewOf('cloudui', 'cloud.ui').with($include('*'))))
            .toLikeC4Model();
    ...
    it('should set summary and description', async ({ expect }) => {
        const m = spec
            .model(({ component }, _) => _(component('c1', {
            summary: 'summary',
            description: {
                md: 'description',
            },
        })))
            .deployment(({ node, instanceOf }, _) => _(node('n1', {
            summary: 'n1-summary',
            description: {
                md: 'n1-description',
            },
    ...
    it('should preserve icon style properties in models', () => {
        const m = spec
            ...
            .views(({ view, $include, $style }, _) => _(view('index', 'Index').with($include('*'), $style('c1', {
            iconColor: 'red',
            iconSize: 'sm',
            iconPosition: 'left',
        }))))
    ...
    it('should support mutliprojects', async ({ expect }) => {
        const m = spec
            .model(({ component, rel }, _) => _(component('c1'), component('c1.sub'), component('@projectB.c1'), component('@projectB.c1.sub'), rel('c1.sub', '@projectB.c1.sub')))
            .views(({ view, $rules, $include }, _) => _(view('index', $rules($include('*'), $include('@projectB.c1.*')))))
            .toLikeC4Model();

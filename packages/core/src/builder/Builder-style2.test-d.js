...
test('Builder types - style 2', () => {
    const b1 = Builder
        ...
        .model(({ actor, system, component, relTo }, _) => _(actor('alice'), actor('bob'), system('cloud').with(component('backend').with(component('api'), component('db', {
        metadata: {
            // @ts-expect-error
            unknown: ['value1'],
            key1: ['value1'],
    ...
    // @ts-expect-error
    relTo('non-existing')))))
        .deployment(({ env, vm, instanceOf }, _) => _(env('prod').with(vm('vm1'), vm('vm2')), env('dev').with(vm('vm1'), instanceOf('cloud.backend.api'), instanceOf('wrong', 
    // @ts-expect-error
    'non-existing'), vm('vm2'))))
        // Test Element View
        .views(({ view, $include, $style }, _) => _(
    // rules inside
    view('view', 
    // @ts-expect-error
    $include('wrong'), 
    ...
    $style('wrong', { color: 'red' }))))
        // Test one view
        .views(_ => _.view('one-view-per-block', _.$rules(
    // @ts-expect-error
    _.$include('wrong'))))
        // Test Element View Of
        .views(({ viewOf, $rules, $include }, _) => _(viewOf('view-of', 'cloud.backend'), 
    // @ts-expect-error
    viewOf('view-of', 'wrong'), viewOf('view-of', 'cloud.backend.api', $rules(
    // @ts-expect-error
    $include('wrong'))), viewOf('view-of', 'cloud.backend.api', $rules(
    // @ts-expect-error
    $include('wrong'))).with($include('* -> alice.*'), 
    // @ts-expect-error
    $include('wrong')), viewOf('view-of', 
    // @ts-expect-error
    'cloud.backensd.api', 'Title').with($include('* -> alice.*'), 
    // @ts-expect-error
    $include('wrong'))))
        // Test Deployment View
        .views(({ deploymentView, $rules, $include }, _) => _(deploymentView('deployment', $rules(
    // @ts-expect-error
    ...
    const b2 = Builder.specification(m.specification)
        .model(({ actor }, _) => _(actor('alice2'), actor('bob2')))
        .deployment(({ env, instanceOf }, _) => _(env('out').with(instanceOf('bob2'))));
...
test('Builder tags - style 2', () => {
    ...
    const b2 = b1
        .model(({ element, relTo }, _) => _(element('a.b2').with(element('c'), relTo('a.b', {
        // @ts-expect-error
        tags: ['o'],
    }), 

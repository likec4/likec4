...
import { type DeploymentFqn, type Expression, type Fqn } from '../types2'
...
export function $deploymentExpr<T extends AnyTypes>(
  ...
  if (expr.endsWith('._')) {
    return asTypedDeploymentExpression({
      ref: {
        deployment: expr.replace('._', '') as DeploymentFqn,
      },
      selector: 'expanded',
    })
  }
  if (expr.endsWith('.**')) {
    return asTypedDeploymentExpression({
      ref: {
        deployment: expr.replace('.**', '') as DeploymentFqn,
      },
  ...
  return asTypedDeploymentExpression({
    ref: {
      deployment: expr as DeploymentFqn,
    },
  })
}

...
namespace EdgeModel {
  export interface StepEdge<M extends AnyAux, V extends ProcessedView<M> = ProcessedView<M>> extends EdgeModel<M, V> {
    // id: StepEdgeId
    stepNumber: number
  }
  export interface WithParent<M extends AnyAux, V extends ProcessedView<M> = ProcessedView<M>> extends EdgeModel<M, V> {

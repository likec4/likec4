...
import { buildComputedNodes } from './buildComputedNodes';
...
describe('topologicalSort', () => {
    // #region
    function testmodel(nodes, edges = []) {
        const nodesMap = buildComputedNodes(LikeC4Model.EMPTY.$styles, nodes.map(id => ({ id, title: id })));
    ...
    function expectOrder(nodes, edges = []) {
    ...
    function expectNodesOrder(nodes, edges = []) {
        const sorted = testmodel(nodes, edges);

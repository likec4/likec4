...
export function topologicalSort(param) {
    ...
    const enrichedEdges = pipe(edges, map((edge) => {
    ...
    const [edgesBetweenLeafs, edgesWithCompounds] = partition(enrichedEdges, ({ source, target }) => source.children.length === 0 && target.children.length === 0);
    const sortedEdges = [];
    ...
    for (const { edge, source, target } of edgesBetweenLeafs) {
        ...
        if (target.parent && target.parent !== edge.parent) {
            pipe(ancestorsOfNode(target, param.nodes), takeWhile(ancestor => ancestor.inEdges.includes(edge.id)), forEach(ancestor => {

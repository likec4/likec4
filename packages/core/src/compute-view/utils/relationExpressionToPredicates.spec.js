...
describe('relationExpressionToPredicates', () => {
    ...
    describe('where', () => {
        it('returns true if condition is met', () => {
            ...
            const predicate = relationExpressionToPredicates($where($relation('support -> cloud'), { tag: { eq: 'aws' } }));
            expect(predicate(matchingRelation)).toBe(true);
        });
        ...
        it('returns true if participant matches', () => {
            ...
            const predicate = relationExpressionToPredicates($where($relation('support -> cloud'), $participant('source', { tag: { eq: 'aws' } })));
            expect(predicate(matchingRelation)).toBe(true);
        });

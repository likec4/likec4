...
describe('treeFromElements', () => {
  ...
  const builder = specs
    .model(({ el }, m) =>
      m(
        el('system'),
        el('system.subsystem1'),
        el('system.subsystem1.component1'),
        el('system.subsystem1.component2'),
        el('system.subsystem2'),
        el('standalone'),
      )
    )
  ...
  it('byId retrieves correct elements', ({ expect }) => {
    ...
    expect(() => tree.byId('nonexistent' as any)).toThrow('Element not found by id: nonexistent')
  })

  it('parent returns correct parent element', ({ expect }) => {
    const tree = treeFromElements(builder.toLikeC4Model().elements())

    const ars = 'system.subsystem1' as const
  ...
  it('handles only roots', ({ expect }) => {
    const result1 = treeFromElements(
      specs
        .model(({ el }, _) =>
          _(
            el('customer'),
  ...
  it('correctly flattens hierarchy', ({ expect }) => {
    ...
    const result3 = treeFromElements(
      specs
        .model(({ el, _ }) =>
          _(
            el('cloud'),
      ...
      specs
        .model(({ el, _ }) =>
          _(
            ...
            el('amazon.rds.pg1'),
            el('amazon.rds.pg2'),
            el('amazon.rds.pg2.db'),
            el('amazon.rds.pg2.db.db1'),
            el('amazon.sqs'),
            el('amazon.sqs.q1'),
            el('amazon.sqs.q2'),
          )

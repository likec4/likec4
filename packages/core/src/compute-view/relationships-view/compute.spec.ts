...
describe('computeRelationshipsView', () => {
  ...
  it('includes child relationships in the subject view', ({ expect }) => {
    const m = builder
        .model(({ _, rel }) =>
          _(
            rel('cloud.frontend', 'cloud.backend.graphql'),
            rel('amazon', 'cloud.backend.graphql'),
            rel('cloud.backend.graphql', 'amazon'),
          )
        )
        .toLikeC4Model(),
      result = computeRelationships('cloud.backend', m, null)
    ...
    const m = builder
        .model(({ _, el, rel }) =>
          _(
            ...
            rel('external.some.service', 'cloud.backend'),
          )
        )
        .toLikeC4Model(),
    ...
    const m = builder
        .model(({ _, el, rel }) =>
          _(
            ...
            rel('cloud.frontend', 'external.some.service'),
          )
        )
        .toLikeC4Model(),
      result = computeRelationships('cloud', m, null)
    ...
    const m = specs
      .model(({ el, rel }, _) =>
        _(
          ...
          rel('cloud.frontend.ui', 'cloud.backend.api'),
        )
      )
      .toLikeC4Model()
  ...
  it('correctly flattens subjects', ({ expect }) => {
    const result1 = computeRelationships(
      'cloud',
      specs
        .model(({ el, rel }, _) =>
          _(
            ...
            el('cloud.backend.service1.api'),
            el('cloud.backend.service2'),
            el('amazon'),
            rel('cloud.backend.service1.api', 'amazon'),
            rel('cloud.backend.service2', 'amazon'),
          )
        )
        .toLikeC4Model(),
      null,
    ...
    const result2 = computeRelationships(
      'cloud',
      specs
        .model(({ el, rel }, _) =>
          _(
            ...
            el('cloud.backend.service1.api'),
            el('cloud.backend.service2'),
            el('cloud.backend.service2.api'),
            el('amazon'),
            rel('cloud.backend.service1.api', 'amazon'),
            rel('cloud.backend.service2.api', 'amazon'),
          )
        )
        .toLikeC4Model(),
      null,
    ...
    const result3 = computeRelationships(
      'cloud',
      specs
        .model(({ el, rel }, _) =>
          _(
            el('cloud'),
            el('cloud.backend1'),
            el('cloud.backend1.service1'),
            el('cloud.backend1.service1.api'),
            el('cloud.backend2'),
            el('cloud.backend2.service2'),
            el('cloud.backend2.service2.api'),
            el('amazon'),
            rel('cloud.backend1.service1.api', 'amazon'),
            rel('cloud.backend2.service2.api', 'amazon'),
          )
        )
        .toLikeC4Model(),
      null,
    ...
    const result4 = computeRelationships(
      'cloud',
      specs
        .model(({ el, rel }, _) =>
          _(
      ...
      specs
        .model(({ el, rel }, _) =>
          _(
            ...
            el('cloud.backend.service2'),
            el('amazon'),
            rel('amazon', 'cloud.backend.service1.api'),
            rel('amazon', 'cloud.backend.service2'),
          )
        )
        .toLikeC4Model(),
      null,
      ...
      specs
        .model(({ el, rel }, _) =>
          _(
            ...
            el('cloud.backend2.service2.api'),
            el('amazon'),
            rel('amazon', 'cloud.backend1.service1.api'),
            rel('amazon', 'cloud.backend2.service2.api'),
          )
        )
        .toLikeC4Model(),
      null,

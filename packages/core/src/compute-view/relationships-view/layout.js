...
function createNodes(prefix, elements, g) {
    const graphNodes = new DefaultMap(key => ({
        id: `${prefix}-${key}`,
        portId: `${prefix}-${key}`,
    }));
...
export function layoutRelationshipsView(data) {
    const g = createGraph();
    const incomers = createNodes('in', data.incomers, g), subjects = createNodes('subject', data.subjects, g), outgoers = createNodes('out', data.outgoers, g);
    const edges = [];
    pipe(concat(pipe(toArray(data.incoming), map(r => ({
        id: r.source.id,
    ...
    // Sort by source
    sortParentsFirst), pipe(toArray(data.outgoing), map(r => ({
        id: r.target.id,
    ...
    const _calculatedNodeBounds = pipe(nodeIds, 
    // Compound nodes have different portId
    filter(n => n.id === n.portId), mapToObj(n => [n.id, dagreBounds(n.id)]));
    function nodeBounds(nodeId) {
        return _calculatedNodeBounds[nodeId] ??= pipe(g.children(nodeId) ?? [], filter(id => !id.endsWith(PortSuffix)), map(id => nodeBounds(id)), tap(bounds => {
        ...
        }, { minY: Infinity, maxY: -Infinity }), ({ minY, maxY }) => {
            const { position: { x }, width, } = dagreBounds(nodeId);
    ...
    const nodes = nodeIds.map(({ id }) => {
        ...
        //   }
        // }
        const children = (g.children(id) ?? []).filter(c => !c.endsWith(PortSuffix));

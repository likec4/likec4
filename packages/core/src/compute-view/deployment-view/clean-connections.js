import { dropWhile, filter, flatMap, forEach, groupBy, hasAtLeast, map, pipe, piped, prop, reduce, values, } from 'remeda';
import { DeploymentConnectionModel, differenceConnections, isNestedConnection, mergeConnections, RelationshipsAccum, sortConnectionsByBoundaryHierarchy, } from '../../model';
...
function findCrossBoundarySameSourceOrTarget(connections) {
    ...
    const excludedRelations = new DefaultMap(() => new Set());
    ...
    for (const [relation, sameRelationGroup] of groupedByRelation) {
        ...
        pipe(sameRelationGroup, flatMap(conn => [
            { group: `$source-${conn.source.id}`, conn },
            { group: `$target-${conn.target.id}`, conn },
        ]), groupBy(prop('group')), values(), filter(hasAtLeast(2)), forEach(
        // In each group, sort by hierarchy, first are deepest
        piped(map(prop('conn')), sortConnectionsByBoundaryHierarchy('desc'), 
 ...
 */
export function findCrossBoundaryConnections(connections) {
    ...
    for (const c of connections) {
        ...
        if (source.isDeploymentNode() !== target.isDeploymentNode()) {
            ...
            const toExclude = intersection(connectionModelRelations, nodeInternals);
        ...
        if (source.isDeploymentNode() && target.isDeploymentNode()) {
            ...
            intersection(c.relations.model, source.internalModelRelationships()), intersection(c.relations.model, target.internalModelRelationships()));
            for (const relation of toExclude) {
                excludedRelations.get(c).add(relation);
            }
        }
    }
    return pipe(excludedRelations.entries(), imap(([c, excluded]) => c.update({
        model: excluded,
        deployment: null,
    })), toArray());
}
 ...
 * ```
 */
export function cleanCrossBoundary(connections) {
 ...
 */
export function findRedundantConnections(connections) {
    return pipe(connections, mergeConnections, reduce((redundants, connection) => {
        ...
        for (const c of connections) {
            if (isNestedConnection(c, connection)) {
                redundantAccum = redundantAccum.union(relations.intersect(c.relations));
            }
        }
        if (redundantAccum.nonEmpty) {
            redundants.push(new DeploymentConnectionModel(source, target, redundantAccum));
        }
        return redundants;
    }, []));
}
 ...
 */
export function cleanRedundantRelationships(connections) {
    return differenceConnections(connections, findRedundantConnections(connections));
}

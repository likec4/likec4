    ...
    describe('commit', () => {
      ...
      `);
        ...
        it('should keep connection if only some relations are excluded', () => {
            ...
            const memory2 = stage1.commit();
            const stage2 = new StageExclude(memory2, { wildcard: true });
            const partialConnection = sliceConnection(findConnection(customer, cloud)[0], r => r.target.id == 'cloud.frontend.mobile');
      ...
      `);
        ...
        it('should handle multiple exclusions of the same connection', () => {
            ...
            const memory2 = stage1.commit();
            const stage2 = new StageExclude(memory2, { wildcard: true });
            const partialConnection1 = sliceConnection(findConnection(customer, cloud)[0], r => r.target.id == 'cloud.frontend.mobile');
            const partialConnection2 = sliceConnection(findConnection(customer, cloud)[0], r => r.target.id == 'cloud.frontend.dashboard');
...
function toReadableMemory(memory) {
    return {
...
function sliceConnection(connection, modelPredicate = null, deploymentPredicate = null) {
    return new DeploymentConnectionModel(connection.source, connection.target, new RelationshipsAccum(new Set([...connection.relations.model].filter(modelPredicate ?? (() => true))), new Set([...connection.relations.deployment].filter(deploymentPredicate ?? (() => true)))));
}

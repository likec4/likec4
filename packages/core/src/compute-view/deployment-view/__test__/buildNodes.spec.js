...
describe('deployment node with onlyOneInstance', () => {
    const { builder: b, model: { el, model }, deployment: { node, deployment, ...d }, } = Builder.forSpecification({
        elements: ['el'],
        deployments: ['node'],
    });
    describe('title handling', () => {
        it('should preserve node title from specs', () => {
            const t = TestHelper.from(Builder.specification({
                elements: ['el'],
                deployments: {
                    node: {
                ...
                .deployment(({ node, ...d }, _) => _(node('server').with(d.instanceOf('instance', 'app', {
        ...
        it('should preserve node title when defined', () => {
            const t = TestHelper.from(b.with(model(el('app', {
                title: 'app title from model',
            })), deployment(node('server', { title: 'node title' }).with(d.instanceOf('instance', 'app', {
        ...
        it('should inherit logical title if not present in deployment model', () => {
            const t = TestHelper.from(b.with(model(el('app', {
                title: 'app title from model',
            })), deployment(node('server').with(d.instanceOf('instance', 'app')))));
            const [nd] = t.computeView(t.$include('server')).nodes;
            expect(nd).toMatchObject({
                id: 'server',
                title: 'app title from model',
    ...
    describe('description/summary handling', () => {
        ...
        it('should preserve node description when instance has summary', () => {
            const t = TestHelper.from(b.with(model(el('app', {
                description: 'app description from model',
            })), deployment(node('server', { description: 'server node description' }).with(d.instanceOf('instance', 'app', {
        ...
        it('should use node summary over description when both present', () => {
            const t = TestHelper.from(b.with(model(el('app')), deployment(node('server', {
                summary: 'node summary',
                description: 'node description',
            }).with(d.instanceOf('instance', 'app', {
    ...
    it('verifies #2387', () => {
        const t = TestHelper.from(Builder.specification({
            ...
            .model(({ external_system }, _) => _(external_system('ca', 'Certificate Authority'), external_system('identity', 'Identity Provider')))
            .deployment(({ deployment, kubernetesNamespace, ...d }, _) => _(kubernetesNamespace('cert_manager', { title: 'cert-manager' }).with(d.instanceOf('ca')), 
        // with title same as name, it should inherit from the instance
        kubernetesNamespace('keycloak1', { title: 'keycloak1' }).with(d.instanceOf('identity')), 
        // with title same as name, but expanded (do not inherit)
        kubernetesNamespace('keycloak2', { title: 'keycloak2' }).with(d.instanceOf('identity')), 
        // with title, it should use the title
        kubernetesNamespace('keycloak3', { title: 'Keycloak' }).with(d.instanceOf('identity')))));
        const nodes = indexBy(t.computeView(t.$include('*'), t.$include('cert_manager.*'), t.$include('keycloak2.*')).nodes, n => n.id);

...
describe('cleanCrossBoundaryConnections', () => {
    it('should keep relations inside boundary', () => {
        const { nodeIds, edgeIds } = computeView($include('prod.eu.zone1.*'), $include('prod.eu.zone2.*'));
    ...
    it('should keep relations inside boundary (2)', () => {
        const { nodeIds, edgeIds } = computeView($include('customer'), $include('acc.testCustomer'), $include('prod._'), $include('acc._'));
    ...
    it('should keep relations inside boundary and remove redundant edges', () => {
        const { nodeIds, edgeIds } = computeView($include('prod.eu.zone1'), $include('prod.eu.zone2'), $include('prod.eu.zone1.*'), $include('prod.eu.zone2.*'));
        expect.soft(nodeIds).toEqual([
            'prod.eu.zone1',
            'prod.eu.zone2',
            'prod.eu.zone1.ui',
    ...
    it('should remove redundant edges based on deployment relations', () => {
        const { nodeIds, edgeIds, edges } = computeView($include('prod.*'), $include('prod.eu.db'), $include('prod.us.db'));
        expect.soft(nodeIds).toEqual([
            'prod.eu',
            'prod.eu.db',
            'prod.us',

...
describe('DeploymentRefPredicate', () => {
    it('should include instance and node', () => {
        const { nodeIds, edgeIds } = computeView($include('customer.instance'), $include('prod.eu'));
    ...
    it('should include nodes and edges', () => {
        const { nodeIds, edgeIds, nodes } = computeView($include('customer'), $include('prod.eu.zone1'), $include('prod.eu.zone2'));
        expect(nodeIds).toEqual([
            'customer',
            'prod',
            'prod.eu.zone1',
    ...
    it('should include nodes and edges (preserve order)', () => {
        const { nodeIds, edgeIds } = computeView($include('customer'), $include('prod.eu.zone2'), $include('prod.eu.zone1'), $include('prod'));
        expect(nodeIds).toEqual([
            'customer',
            'prod',
            'prod.eu.zone2',
    ...
    it('should include children', () => {
        const { nodeIds, edgeIds } = computeView($include('prod.eu.*'), $exclude('prod.eu.auth'));
    ...
    it('should include children and ensure sort', () => {
        const { nodeIds, edgeIds } = computeView($include('prod.eu.zone1.*'));
        expect.soft(nodeIds).toEqual([
            'prod.eu.zone1.ui',
            'prod.eu.zone1.api',
        ]);
    ...
    it('should include descendants and ensure sort', () => {
        const { nodeIds, edgeIds, nodes } = computeView($include('prod.eu.**'), $exclude('prod.eu.auth'));
        expect.soft(nodeIds).toEqual([
            'prod.eu.zone1',
            'prod.eu.zone2',
            'prod.eu.zone1.ui',
    ...
    it('should expand node 1', () => {
        const { nodeIds, edgeIds } = computeView($include('customer'), $include('prod.eu.zone1._'));
        expect.soft(nodeIds).toEqual([
            'customer',
            'prod',
            'prod.eu.zone1',
    ...
    it('should expand node 2', () => {
        const view = computeView($include('*'), $include('prod.eu.zone1._'));
    ...
    it('should expand node 3', () => {
        const view = computeView($include('customer'), $include('prod.eu._'), $include('prod.eu.zone2._'), $exclude('prod.eu.auth'));
        expect(view).toMatchObject({
            edgeIds: [
                'customer:prod.eu.zone1',
                'prod.eu.zone1:prod.eu.media',
    ...
    it('nodes should inherit titles from deployed instances', () => {
        const { nodeIds, nodes } = computeView($include('prod.eu.zone1.ui'), $include('prod.eu.zone2.ui'), $include('prod.us.zone1.ui'), $include('acc.eu.ui'));
        expect(nodeIds).toEqual([
            'prod.eu.zone1.ui',
            'prod.eu.zone2.ui',
            'prod.us.zone1.ui',

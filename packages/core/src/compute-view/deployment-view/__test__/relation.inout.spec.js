...
import { $exclude, $include, computeNodesAndEdges } from './fixture';
function expectComputed(...rules) {
...
describe('InoutRelationPredicate', () => {
    it('should include relation incoming to deployment node if it matches condition', () => {
        expectComputed($include('customer'), $include('-> prod.eu.zone1.ui ->', { where: 'tag is #old' })).toMatchInlineSnapshot(`
      {
        "Nodes": [
          "customer",
          "prod",
    ...
    it('should include relation outgoing from deployment node if it matches condition', () => {
        expectComputed($include('prod.eu.zone1.api'), $include('-> prod.eu.zone1.ui ->', { where: 'tag is #old' })).toMatchInlineSnapshot(`
      {
        "Nodes": [
          "prod.eu.zone1.ui",
          "prod.eu.zone1.api",
    ...
    it('should not include relation incoming to deployment node if it does not match condition', () => {
        expectComputed($include('customer'), $include('-> prod.eu.zone1.ui ->', { where: 'tag is #next' })).toMatchInlineSnapshot(`
      {
        "Nodes": [
          "customer",
        ],
    ...
    it('should not include relation outgoing from deployment node if it does not match condition', () => {
        expectComputed($include('prod.eu.zone1.api'), $include('-> prod.eu.zone1.ui ->', { where: 'tag is #next' })).toMatchInlineSnapshot(`
      {
        "Nodes": [
          "prod.eu.zone1.api",
        ],
    ...
    it('should exclude relation incoming to model element if it matches condition', () => {
        expectComputed($include('prod.eu.*'), $include('prod.eu.zone1.**'), $exclude('prod.eu.zone2'), $exclude({
            inout: {
                ref: {
                    model: 'cloud.backend',
                },
            },
        }, { where: 'tag is #old' })).toEqual({
            Nodes: [
                'prod.eu.zone1',
                'prod.eu.zone1.api',
                'prod.eu.zone1.ui',
    ...
    it('should exclude relation incoming to deployment node if it matches condition', () => {
        expectComputed($include('customer'), $include('prod.eu.*'), $include('prod.eu.zone1.**'), $exclude('prod.eu.zone2'), $exclude('-> prod.eu.zone1.* ->', { where: 'tag is #old' })).toEqual({
            Nodes: [
                'customer',
                'prod',
                'prod.eu',
    ...
    it('should exclude relation outgoing from deployment node if it matches condition', () => {
        expectComputed($include('prod.eu.*'), $include('prod.eu.zone1.**'), $exclude('prod.eu.zone2'), $exclude('-> prod.eu.zone1.* ->', { where: 'tag is #temp' })).toEqual({
            Nodes: [
                'prod.eu.zone1',
                'prod.eu.zone1.ui',
                'prod.eu.zone1.api',
    ...
    it('should not exclude relation incoming to deployment node if it does not match condition', () => {
        expectComputed($include('customer'), $include('prod.eu.*'), $include('prod.eu.zone1.**'), $exclude('prod.eu.zone2'), $exclude('-> prod.eu.zone1.* ->', { where: 'tag is #next' })).toEqual({
            Nodes: [
                'customer',
                'prod',
                'prod.eu',
    ...
    it('should not exclude relation outgoing from deployment node if it does not match condition', () => {
        expectComputed($include('prod.eu.*'), $include('prod.eu.zone1.**'), $exclude('prod.eu.zone2'), $exclude('-> prod.eu.zone1.* ->', { where: 'tag is #next' })).toEqual({
            Nodes: [
                'prod.eu.zone1',
                'prod.eu.zone1.ui',
                'prod.eu.zone1.api',

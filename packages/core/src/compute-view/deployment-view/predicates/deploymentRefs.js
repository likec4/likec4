...
export const DeploymentRefPredicate = {
    include: (ctx) => {
    ...
    exclude: ({ expr, stage, memory, where }) => {
        ...
        const toExclude = pipe([...memory.elements], filter(exprPredicate), applyElementPredicate(where));
...
function includeDeployedNodeDescendants(node, { stage, where }) {
    ...
    pipe(descendants, filter(desc => allConnected.has(desc)), forEach(desc => stage.addExplicit(desc)));
}
function findConnectedElements(stage) {
    return pipe(stage.mergedConnections(), cleanCrossBoundary, cleanRedundantRelationships, reduce((acc, c) => {
        acc.add(c.source);
        acc.add(c.target);
        return acc;
    }, new Set()));

...
import { deploymentExpressionToPredicate, findConnection, findConnectionsBetween, findConnectionsWithin, resolveElements, resolveModelElements, } from '../utils';
...
//   })
// }
export const DirectRelationPredicate = {
    include: ({ expr: { source, target, isBidirectional = false }, model, stage, where }) => {
        ...
        switch (true) {
            ...
            case sourceIsWildcard && targetIsWildcard: {
                connections = pipe(findConnectionsWithin(model.instances()), applyPredicate(where), map(c => {
            ...
            case !sourceIsWildcard && targetIsWildcard: {
                ...
                connections = pipe(sources, flatMap(source => {
                    const targets = resolveAscendingSiblings(source);
                    return findConnectionsBetween(source, targets, dir);
                }), filter(postFilter), applyPredicate(where));
                break;
            }
            // * -> target; * <-> target
            case sourceIsWildcard && !targetIsWildcard: {
                ...
                connections = pipe(targets, flatMap(target => {
            ...
            default: {
                ...
                const isSource = filterOutgoingConnections(sources);
                const isTarget = filterIncomingConnections(targets);
                connections = pipe(sources, flatMap(s => findConnectionsBetween(s, targets, dir)), filter(c => isSource(c) && isTarget(c)), applyPredicate(where));
            }
        }
    ...
    exclude: ({ expr, model, memory, stage, where }) => {
        ...
        switch (true) {
            ...
            case FqnExpr.isModelRef(expr.source):
                modelRelationsToExclude = resolveAllOutgoingRelations(model, expr.source);
                return excludeModelRelations(modelRelationsToExclude, { stage, memory }, where, c => isTarget(c.target));
            // deployment -> model
            case FqnExpr.isModelRef(expr.target):
                modelRelationsToExclude = resolveAllImcomingRelations(model, expr.target);
                return excludeModelRelations(modelRelationsToExclude, { stage, memory }, where, c => isSource(c.source));
            ...
            default:
                ...
                const deploymentRelationsToExclude = pipe(memory.connections, filter(satisfies), applyPredicate(where));

...
export const OutgoingRelationPredicate = {
    ...
    exclude: ({ expr, model, memory, stage, where }) => {
        ...
        const toExclude = pipe(memory.connections, filter(isOutgoing), applyPredicate(where));
        stage.excludeConnections(toExclude);
        return stage;
    },
};
export function filterOutgoingConnections(sources) {
    return anyPass(sources.map(source => {
...
export function resolveAllOutgoingRelations(model, moodelRef) {

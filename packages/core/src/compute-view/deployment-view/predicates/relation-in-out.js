...
export const InOutRelationPredicate = {
    ...
    exclude: ({ expr, model, memory, stage, where }) => {
        ...
        if (FqnExpr.isModelRef(expr.inout)) {
            ...
            const excludedRelations = union(new Set(), ...elements.flatMap(e => [e.allIncoming, e.allOutgoing]));
        ...
        const toExclude = pipe(memory.connections, filter(c => isIncoming(c) !== isOutgoing(c)), applyPredicate(where));

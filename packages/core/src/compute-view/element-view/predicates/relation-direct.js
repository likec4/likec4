...
import { findConnectionsBetween, findConnectionsWithin, includeDescendantsFromMemory, resolveAndIncludeFromMemory, resolveElements, } from './_utils';
const isWildcard = ModelFqnExpr.isWildcard;
const isAncestorOrDescendantOf = (a, b) => a.isAncestorOf(b) || a.isDescendantOf(b);
export const DirectRelationExprPredicate = {
    include: ({ expr: { source, target, isBidirectional = false }, memory, model, stage, where, filterWhere }) => {
        ...
        switch (true) {
            ...
            case sourceIsWildcard && targetIsWildcard && memory.isEmpty() && where !== NoWhere: {
                const connections = pipe(model.relationships(), ifilter(where), toArray(), map(r => new ConnectionModel(r.source, r.target, new Set([r]))));
            ...
            case !sourceIsWildcard && targetIsWildcard && where !== NoWhere: {
                ...
                const connections = pipe(sources, flatMap(source => pipe(source, when(constant(isBidirectional === true), {
                    onTrue: s => union(s.allIncoming, s.allOutgoing),
                    onFalse: s => s.allOutgoing,
                }), ifilter(where), iunique(), toArray(), partition(r => r.source === source), ([outgoing, incoming]) => concat(pipe(outgoing, filter(r => !isAncestorOrDescendantOf(source, r.target)), map(outgoing => new ConnectionModel(source, outgoing.target, new Set([outgoing])))), pipe(incoming, filter(r => !isAncestorOrDescendantOf(source, r.source)), map(incoming => new ConnectionModel(incoming.source, source, new Set([incoming]))))))));
                stage.addConnections(connections);
                return stage;
            }
            // element -> *
            case !sourceIsWildcard && targetIsWildcard: {
                ...
                for (const source of sources) {
                    connections.push(...findConnectionsBetween(source, targets, dir));
                }
                break;
            }
            // This is a special case, we look for all relationships that satisfy the where clause
            ...
            case sourceIsWildcard && !targetIsWildcard && where !== NoWhere: {
                ...
                const connections = pipe(targets, flatMap(target => pipe(target, when(constant(isBidirectional === true), {
                    onTrue: s => union(s.allIncoming, s.allOutgoing),
                    onFalse: s => s.allIncoming,
                }), ifilter(where), toArray(), partition(r => r.target === target), ([incoming, outgoing]) => concat(pipe(outgoing, filter(r => !isAncestorOrDescendantOf(target, r.target)), map(outgoing => new ConnectionModel(target, outgoing.target, new Set([outgoing])))), pipe(incoming, filter(r => !isAncestorOrDescendantOf(target, r.source)), map(incoming => new ConnectionModel(incoming.source, target, new Set([incoming]))))))));
                stage.addConnections(connections);
                return stage;
            }
            // * -> element
    ...
    exclude: ({ expr: { source, target, isBidirectional }, model, memory, stage, where }) => {
        ...
        switch (true) {
            // * -> *
            case sourceIsWildcard && targetIsWildcard: {
                relations = pipe(memory.connections, flatMap(piped(prop('relations'), ifilter(where), toArray())), toSet());
                break;
            }
            // element -> *
            case !sourceIsWildcard && targetIsWildcard: {
                const sources = resolveElements(model, source);
                relations = pipe(sources, flatMap(source => pipe(source, when(constant(isBidirectional === true), {
                    onTrue: s => union(s.allIncoming, s.allOutgoing),
                    onFalse: s => s.allOutgoing,
            ...
            case sourceIsWildcard && !targetIsWildcard: {
                const targets = resolveElements(model, target);
                relations = pipe(targets, flatMap(target => pipe(target, when(constant(isBidirectional === true), {
                    onTrue: s => union(s.allIncoming, s.allOutgoing),
                    onFalse: s => s.allIncoming,
                }), ifilter(where), toArray())), toSet());
                break;
            ...
            default: {
                ...
                for (const source of sources) {
                    for (const target of targets) {
                        ...
                        accum = union(accum, intersection(source.allOutgoing, target.allIncoming), isBidirectional ? intersection(target.allOutgoing, source.allIncoming) : new Set());
                    }
                }
...
function resolveWildcard(nonWildcard, { memory, model }) {
    ...
    const targets = pipe(sources, map(el => el.ascendingSiblings()), iflat(), iunique(), toArray(), all => includeDescendantsFromMemory(all, memory));

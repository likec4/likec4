...
export const IncomingExprPredicate = {
    include: ({ expr, scope, model, memory, stage, filterWhere }) => {
        ...
        if (ModelFqnExpr.isWildcard(target)) {
            ...
            for (const sibling of scope.ascendingSiblings()) {
                connections.push(...findConnection(sibling, scope, 'directed'));
            }
        }
        else {
            ...
            if (visibleElements.length === 0) {
                visibleElements.push(...unique(targets.flatMap(el => [...el.ascendingSiblings()])));
            }
            const ensureIncoming = incomingConnectionPredicate(model, target);
            for (const visible of visibleElements) {
                connections.push(...findConnectionsBetween(visible, targets, 'directed').filter(ensureIncoming));
            }
        }
...
export function incomingConnectionPredicate(model, expr) {
    switch (true) {
        ...
        case ModelFqnExpr.isModelRef(expr) && expr.selector === 'children': {
            ...
            return anyPass([...model.children(fqn)].map(el => Connection.isIncoming(el.id)));
        }
        case ModelFqnExpr.isModelRef(expr) && expr.selector === 'descendants': {
            const fqn = FqnRef.flatten(expr.ref);
            return anyPass([
                Connection.isInside(fqn),
                ...[...model.children(fqn)].map(el => Connection.isIncoming(el.id)),
            ]);
        }

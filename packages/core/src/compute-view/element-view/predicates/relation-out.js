...
export const OutgoingExprPredicate = {
    include: ({ expr, scope, model, memory, stage, filterWhere }) => {
        ...
        if (ModelFqnExpr.isWildcard(target)) {
            ...
            if (visibleElements.length === 0) {
                visibleElements.push(...unique(elements.flatMap(el => [...el.ascendingSiblings()])));
            }
            const ensureOutgoing = outgoingConnectionPredicate(model, target);
            for (const source of elements) {
                connections.push(...findConnectionsBetween(source, visibleElements, 'directed').filter(ensureOutgoing));
            }
        }
...
export function outgoingConnectionPredicate(model, expr) {
    switch (true) {
        ...
        case ModelFqnExpr.isModelRef(expr) && expr.selector === 'children': {
            ...
            return anyPass([...model.children(fqn)].map(el => Connection.isOutgoing(el.id)));
        }
        case ModelFqnExpr.isModelRef(expr) && expr.selector === 'descendants': {
            const fqn = FqnRef.flatten(expr.ref);
            return anyPass([
                Connection.isInside(fqn),
                ...[...model.children(fqn)].map(el => Connection.isOutgoing(el.id)),
            ]);
        }

...
function expectParents(nodes) {
    return expect(pipe(nodes, indexBy(prop('id')), mapValues(prop('parent'))));
}
describe('groups', () => {
    it('should include elements', () => {
        ...
        const [group1, support, group2, cloud] = nodes;
    ...
    it('nested groups', () => {
        ...
        // parent of each node
        expect(pipe(nodes, indexBy(prop('id')), mapValues(prop('parent')))).toMatchInlineSnapshot(`
      {
        "@gr1": null,
        "@gr2": "@gr1",
        "@gr3": "@gr1",
    ...
    `);
        ...
        expect(pipe(nodes, indexBy(prop('id')), mapValues(prop('children')))).toMatchInlineSnapshot(`
      {
        "@gr1": [
          "@gr4",
          "@gr2",
    ...
    `);
        ...
        // parents of edges
        expect(pipe(edges, map(e => ({
            ...e,
            id: `${e.source}:${e.target}`,
        })), indexBy(prop('id')), mapValues(prop('parent')))).toMatchInlineSnapshot(`
      {

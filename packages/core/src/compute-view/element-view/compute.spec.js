...
describe('compute', () => {
    it('adds tags to node from element and spec', () => {
        const context = compute(builder
            .model(({ actor }, _) => _(actor('alice'), actor('bob', {
            tags: ['tag2'],
        })))
            .views(({ view, $include, $rules, $style }, _) => _(view('index', 'index', $rules($include('*'), $style(['*', 'alice'], {}))))));
    ...
    it('pushes edge to outbound collection of the source and its ancestors', () => {
        const context = compute(builder
            .model(({ el, rel }, _) => _(el('cloud').with(el('backend').with(el('graphql'), el('db'))), el('amazon'), rel('cloud.backend.graphql', 'amazon')))
            .views(({ view, $include, $rules }, _) => _(view('index', 'index', $rules($include('cloud'), $include('cloud.*'), $include('cloud.backend.*'), $include('amazon'))))));
    ...
    it('does not push edge to in- outbounds of the closest common ancestor', () => {
        const context = compute(builder
            .model(({ el, rel }, _) => _(el('cloud'), el('cloud.backend'), el('cloud.backend.graphql'), el('cloud.frontend'), el('cloud.frontend.dashboard'), rel('cloud.backend.graphql', 'cloud.frontend.dashboard')))
            .views(({ view, $include, $rules }, _) => _(view('index', 'index', $rules($include('cloud'), $include('cloud.*'), $include('cloud.backend.*'), $include('cloud.frontend.*'))))));
    ...
    it('pushes edge to inbound collection of the target and its ancestors', () => {
        const context = compute(builder
            .model(({ el, rel }, _) => _(el('cloud'), el('cloud.backend'), el('cloud.backend.graphql'), el('amazon'), rel('amazon', 'cloud.backend.graphql')))
            .views(({ view, $include, $rules }, _) => _(view('index', 'index', $rules($include('cloud'), $include('cloud.*'), $include('cloud.backend.*'), $include('amazon'))))));
    ...
    it('collects rank constraints from rules', () => {
        const parsed = builder
            .model(({ actor }, _) => _(actor('alice'), actor('bob')))
            .views(({ view, $include, $rules }, _) => _(view('index', 'index', $rules($include('*')))))
            .build();
        const likec4model = LikeC4Model.create(parsed);
        const view = only(values(parsed.views));
        invariant(view && isElementView(view), 'Must have one element view');
        ...
        const context = withReadableEdges(computeElementView(likec4model, {
            ...view,

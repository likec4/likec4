...
import { differenceConnections, findDeepestNestedConnection, findDescendantConnections, isAnyInOut, isIncoming, isOutgoing, mergeConnections, } from '../../model';
import { intersection, union } from '../../utils';
import { ifilter, toSet } from '../../utils/iterable';
export function allRelationshipsFrom(connections) {
...
export function findRedundantConnections(connections) {
    return pipe([...connections], mergeConnections, reduce((reducedConnections, connection, _, all) => {
        const descendants = findDescendantConnections(all, connection);
        const nestedRelations = union(...descendants.map(prop('relations')));
        ...
        if (accum.size < connection.relations.size) {
            ...
            if (isSourceExpanded) {
                accum = union(accum, toSet(ifilter(connection.relations, isOutgoing(connection.source))));
            }
            // Check if target expandaded
            if (isTargetExpanded) {
                accum = union(accum, toSet(ifilter(connection.relations, isIncoming(connection.target))));
            }
        }
        if (accum.size > 0) {

import type { LanguageClient, LayoutFn } from '$/di'
import { di } from '$/di'
import { ADisposable, disponsable, queueMicrotask } from '$/util'
...
export class C4ModelImpl extends ADisposable {
  ...
  constructor(
    ...
    this._register(disponsable(() => {
      this.onDidChangeSubscription?.dispose()
    }))
  }

  private modelStream = xs.create<number>({
    start: (listener) => {
  ...
  public fetchModel() {
    return xs.fromPromise(this.client.sendRequest(fetchLikeC4Model))
      .map(s => s.model)
      .filter(isNotNullish)
  }
  ...
  public subscribeToView(viewId: ViewID, callback: (diagram: LayoutedView) => void) {
    ...
    return this._register(disponsable(() => {
      queueMicrotask(() => {

...
import { useHoveredNodeId } from './state'
...
export function Compounds({ animate, theme, diagram, onNodeClick }: CompoundsProps) {
  ...
  const compoundTransitions = useTransition(nodes, {
    ...
    delay(key) {
      ...
      return animate && isUpdating ? 30 : 0
    },
  ...
  return compoundTransitions((springs, node, { key }) => (
    <CompoundShape key={key} node={node} theme={theme} springs={springs} onNodeClick={onNodeClick} />
  ))
}

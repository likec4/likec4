...
export const NodeTitle = memo(
  ({ title, description, color, width, theme, ...props }: NodeTitleProps) => {
    ...
    const [[titleHeight, descHeight], setHeights] = useState([
      ...
      hasDescription
        ? Sizes.description.lineHeight * Sizes.description.fontSize + Sizes.padding * 2
        : 0
    ] as const)
    ...
    return (
      <Group {...props} offsetY={offsetY} _useStrictMode>
        <Text
          ref={titleRef}
          x={0}
          y={0}

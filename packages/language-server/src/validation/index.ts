...
import {
  checkDynamicActivateActorExists,
  checkDynamicAutonumberStep,
  checkDynamicBlockNestingDepth,
  checkDynamicCreateActorExists,
  checkDynamicCreateBeforeFirstUse,
  checkDynamicDanglingDeactivate,
  checkDynamicDeactivateActorExists,
  checkDynamicDestroyActorExists,
  checkDynamicDestroyIsLastUse,
  checkDynamicEmptyBlockBody,
  checkDynamicNoteActorsExist,
  checkDynamicNoteOverActors,
  checkDynamicParallelMixedChildren,
  checkDynamicSequenceOnlyInDiagramVariant,
...
const isValidatableAstNode = isAnyOf(
  ...
  ast.isDynamicNote,
  ast.isDynamicActivate,
  ast.isDynamicDeactivate,
  ast.isDynamicCreate,
  ast.isDynamicDestroy,
  ast.isDynamicAutonumberProperty,
  ast.isDynamicBlockBody,
  ast.isDynamicView,
...
export function registerValidationChecks(services: LikeC4Services) {
  ...
  registry.register<ast.LikeC4AstType>({
    ...
    DynamicNote: [checkDynamicNoteActorsExist(services), checkDynamicNoteOverActors(services)],
    DynamicActivate: checkDynamicActivateActorExists(services),
    DynamicDeactivate: [checkDynamicDeactivateActorExists(services), checkDynamicDanglingDeactivate(services)],
    DynamicCreate: [checkDynamicCreateActorExists(services), checkDynamicCreateBeforeFirstUse(services)],
    DynamicDestroy: [checkDynamicDestroyActorExists(services), checkDynamicDestroyIsLastUse(services)],
    DynamicViewParallelSteps: [dynamicViewParallelSteps(services), checkDynamicParallelMixedChildren(services)],
    DynamicAutonumberProperty: checkDynamicAutonumberStep(services),
    DynamicView: checkDynamicSequenceOnlyInDiagramVariant(services),
    DynamicBlockBody: [checkDynamicEmptyBlockBody(services), checkDynamicBlockNestingDepth(services)],

...
describe('LikeC4ManualLayouts', () => {
    ...
    function createMockView(overrides) {
        const base = {
            ...
            hash: 'hash1',
            autoLayout: { direction: 'TB', nodeSep: 60, rankSep: 50 },
            nodes: [{
                    id: 'node1',
                    parent: null,
                    title: 'Node 1',
    ...
    function mockFsReads(fs, ...views) {
        vi.mocked(fs.scanDirectory).mockResolvedValue(views.map(view => ({
    ...
    describe('read', () => {
        ...
        it('should respect custom manualLayouts outDir config', async ({ expect }) => {
            ...
            expect(fs.scanDirectory).toHaveBeenCalledWith(expect.objectContaining({

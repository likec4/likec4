...
describe('MCP server integration - tools expose links', () => {
    ...
    `);
        ...
        const mcpFactory = new MCPServerFactory({
            likec4: { LanguageServices: services.likec4.LanguageServices },
            shared: { lsp: {} },
        });
        ...
        const result = await handler({ id: 'cloud', project: 'default' }, {});
    ...
    `);
        ...
        const result = await handler({ id: 'dc', project: 'default' }, {});
        expect(result.structuredContent).toBeDefined();
        const links = result.structuredContent['links'];
        expect(Array.isArray(links)).toBe(true);

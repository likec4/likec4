...
views {
  ...
  it('inserts a style inside an empty view that extends another view', async ({ expect }) => {
    const { change, read } = await testDoc(
      expect,
      `
specification {
...
views {
  ...
  it('inserts a style inside a deployment view with only non-style rules', async ({ expect }) => {
    const { change, read } = await testDoc(
      expect,
      `
specification {

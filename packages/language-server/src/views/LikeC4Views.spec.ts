...
describe('LikeC4Views', () => {
  ...
  it('layoutedModel preserves manual layout snapshots', async ({ expect }) => {
    const { validate, services } = createTestServices()
    await validate(`
      specification {
        element component
    ...
    const autoDiagram = await services.likec4.Views.diagrams().then(diagrams => diagrams[0])
    ...
    services.shared.workspace.WorkspaceManager.forceCleanCaches()

    const layoutedModel = await services.likec4.LanguageServices.layoutedModel()
  ...
  it('layoutedModel preserves applied manual layout drift reasons', async ({ expect }) => {
    const { validate, services } = createTestServices()
    await validate(`
      specification {
        element component
    ...
    services.shared.workspace.WorkspaceManager.forceCleanCaches()
    vi.spyOn(services.likec4.Views, 'diagrams').mockResolvedValue([manualDiagram])
  ...
  it('layoutedModel keeps auto views when another view has manual layout', async ({ expect }) => {
    const { validate, services } = createTestServices()
    await validate(`
      specification {
        element component
    ...
    const diagrams = await services.likec4.Views.diagrams().then(d => indexBy(d, v => v.id as 'index' | 'sys2'))

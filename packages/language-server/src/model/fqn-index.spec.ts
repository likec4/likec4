...
function asserts(services: LikeC4Services, expect: ExpectStatic) {
  return {
    expectDirectChildrenOf(fqn: string) {
      const children = services.likec4.FqnIndex.directChildrenOf(fqn as Fqn).toArray().map(pick(['name', 'fqn']))
      return expect(children)
    },
    expectDescendantsOf(fqn: string) {
      const children = services.likec4.FqnIndex.uniqueDescedants(fqn as Fqn).toArray().map(pick(['name', 'fqn']))
      return expect(children)
    },
  }
}

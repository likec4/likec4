import { isViewRulePredicate, ModelRelationExpr, } from '@likec4/core';
...
describe('LikeC4ModelParser', () => {
    ...
    describe('logical model', () => {
      ...
      `);
            ...
            const doc = services.likec4.ModelParser.parse(document);
            const views = indexBy(doc.c4Views, ({ id }) => id);
    ...
    describe('relation predicate', () => {
        ...
        `);
            ...
            const includeRule = rules[0];
            const withPredicate = includeRule.include?.[0];
        ...
        it('"where"', async ({ expect }) => {
          ...
          `);
            ...
            const rules = doc?.c4Views?.[0]?.rules;
            const includeRule = rules[0];
            const wherePredicate = includeRule.include?.[0];
            expect(isViewRulePredicate(includeRule)).toBe(true);

...
function asserts(services, expect) {
    return {
        expectDirectChildrenOf(fqn) {
            const projectId = services.shared.workspace.ProjectsManager.defaultProjectId;
            const children = services.likec4.FqnIndex.directChildrenOf(projectId, fqn).toArray().map(({ name, id }) => ({
                name,
                fqn: id,
            }));
            return expect(children);
        },
        expectDescendantsOf(fqn) {
            const projectId = services.shared.workspace.ProjectsManager.defaultProjectId;
            const children = services.likec4.FqnIndex.uniqueDescedants(projectId, fqn).toArray().map(({ name, id }) => ({
                name,
                fqn: id,
            }));
            return expect(children);

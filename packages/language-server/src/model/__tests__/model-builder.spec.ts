...
describe('LikeC4ModelBuilder', () => {
  ...
  it('builds model with extend', async ({ expect }) => {
    ...
    // 3 explicit views (implicitViews is disabled by default)
  ...
  it('merges relationship spec tags with own tags (unique)', async ({ expect }) => {
    const { validate, buildModel } = createTestServices()
    const { diagnostics } = await validate(`
    specification {
      element person
  ...
  it('relationship spec tags work in view predicates', async ({ expect }) => {
    const { validate, buildModel } = createTestServices()
    const { diagnostics } = await validate(`
    specification {
      element person
  ...
  it('builds deployment relationship inheriting tags from specification', async ({ expect }) => {
    const { validate, buildModel } = createTestServices()
    const { diagnostics } = await validate(`
    specification {
      element component

...
- `packages/likec4/src/LikeC4.ts` — SDK entry (`import { LikeC4 } from 'likec4'`)
- `packages/likec4/src/index.ts` — SDK re-exports (includes `writeDSL`)
- `packages/language-services/src/common/LikeC4.ts` — the real `LikeC4` class
- `packages/language-services/src/node/index.ts` — Node-only helpers (`fromWorkspace`, `fromSource`, `writeDSL`)
- `packages/core/src/builder/Builder.ts` — `Builder` (statics: `forSpecification`/`specification`/`fromParsed`)
- `packages/core/src/builder/_types.ts` — Builder type machinery (`Types`, `AnyTypes`, `Types.FromAux`, `Types.ToAux`)
- `packages/core/src/model/LikeC4Model.ts` — `LikeC4Model` (Parsed/Computed/Layouted)
- `packages/core/src/types/model-data.ts` — `ParsedLikeC4ModelData`, `ComputedLikeC4ModelData`, `LayoutedLikeC4ModelData`
- `packages/generators/src/likec4/index.ts` — DSL emitter (`generate(input): string`); production usage in `packages/language-server/src/model-change/viewChange.ts`

### Model stages

`ParsedLikeC4ModelData` (parsed from DSL) → `ComputedLikeC4ModelData` (views computed via `computeLikeC4Model`) → `LayoutedLikeC4ModelData` (Graphviz positions applied). All three share `elements`/`relations`/`deployments`/`specification`/`globals`/`imports`; only `views` differs. `LikeC4Model.Parsed.$data` is the canonical input to `Builder.fromParsed`.

### Composite TS project gotcha

- `packages/core` has `composite: true`. Downstream packages (`language-services`, `likec4`, `generators`) read `.d.ts` from `packages/core/lib/`, NOT source.
- After adding a new export/method in core, run `pnpm exec tsc --build` (or `pnpm generate`) before typechecking downstream — otherwise tsc reports phantom "Property X does not exist on Y" errors.
- If downstream `tsc -b` shows errors that source-mode `tsc --noEmit` (in the package dir) does not, clear stale tsbuildinfo: `find packages -name "*.tsbuildinfo" -delete` then rebuild.

### Builder type-loss when loading from runtime data

- `Builder.fromParsed(data)` is overloaded: typed `Aux` data (e.g. `builder.build()` output) preserves types via `Types.FromAux<A>`; data with `UnknownParsed` Aux (e.g. from `LikeC4.toBuilder()`) yields a `Builder` whose helpers are runtime-only.
- Callers who know the spec statically can supply an explicit generic: `Builder.fromParsed<typeof mySpec['Types']>(data)` — unchecked cast, caller's responsibility.
...
The DSL generator at `packages/generators/src/likec4/` (`generate(input): string`) emits formatted `.c4` source from `ParsedLikeC4ModelData`, but does NOT preserve comments, source positions, or original formatting. Used by `LikeC4.toDSL()` and `writeDSL()`. Treat round-trip (`fromWorkspace` → `toDSL` → `fromSource`) as one-way generation, not a formatter.

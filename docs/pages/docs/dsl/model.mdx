...
The singe source of truth for the architecture is the model. It is a set of elements and relations between them.
...
Element name is also used as a default title and displayed on views.  
To customize it.
...
This way you define the structure, the internals if the element.
...
Element name must be unique within the container (the same hierarchy level).  

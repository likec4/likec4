...
<Info>
  `index` is a special view, and is rendered by default if no view name is specified. If not defined, it will be
  generated for top-level elements
</Info>
...
<Info>
  Rules order of the rules is significant (especially for "implicit" relationships) Play with the order to get the
  desired result.
</Info>
...
<Info>[Custom properties](#custom-properties) have higher priority than style rules.</Info>

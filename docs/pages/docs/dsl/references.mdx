...
Every element creates a new scope with nested elements, so `api` is unique in the scope of `service1` and `service2`,
but not in the scope of `model`.  

We can fix it by moving the relation to the scope of `service2`:
...
Full qualified name is a unique "path" to the element if we traverse all the parents.

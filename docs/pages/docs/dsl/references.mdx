...
The element name is hoisted (or _"bubbles"_) to the upper scope while it stays unique.  
We even may reference something that is not yet defined but will be hoisted later.

The relationship from `frontend` referencing to not yet defined `graphql`:
...
<Callout type="info" emoji="ℹ️">
Line 7 and line 17 are the same:  
`frontend -> api`

But they refer to different elements:  
...
Fully qualified name is a global name, a "unique path" to an element if we traverse its parents.
...
Middle parts may be omitted, if the target element is unique:

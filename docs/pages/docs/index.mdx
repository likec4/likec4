...
and generate multiple architecture diagrams from different perspectives, like per use-case.
And any change, any refinement or deprecation in the model is automatically reflected,
keeping diagrams up-to-date and synchronised with the actual state.
...
LikeC4 inherits the simplicity of the C4 Model, with focus on Architecture as Code approach, importing/exporting to various formats and documentation tools.
...
- change [shape](/docs/dsl/specification/#shapes) of the `customer` element
- add database (with `storage` shape) and tables like `customers` and `orders` (what relationships should be added?)
- add external system, like Stripe, and show how the backend interacts with it.
